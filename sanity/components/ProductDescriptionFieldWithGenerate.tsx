...
export function ProductDescriptionFieldWithGenerate(props: TextInputProps) {
  ...
  const canGenerate = canGenerateDescription({
    category,
    technical,
  });

  const handleGenerate = () => {
    const generated = generateProductDescription({
      category,
      technical,
    });
    if (generated) {
  ...
  return (
    <div>
      ...
      {!canGenerate && (
        ...
        >
          Fill in Category and Technical details to enable generation
        </div>
      )}
    </div>
  );
